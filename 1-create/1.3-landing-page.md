...
Start with a minimal Next.js app in v0 and wire Clerk for authentication:

1. **Create a new v0 Project**:
   - In v0, click `Create Project` and select a minimal Next.js template

2. **Install and configure Clerk**:
   - Add environment variables in Project Settings: `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`
   - Install `@clerk/nextjs`
   - Add `middleware.ts` with Clerk middleware
   - Wrap `app/layout.tsx` with `ClerkProvider`
   - Add basic routes for Sign In/Sign Up using Clerk hosted components or `<SignIn/>`/`<SignUp/>`

3. **Optional: Convex waitlist**
   - If you prefer email-only waitlist capture before full auth, create a simple `/waitlist` page that calls a Convex mutation to store emails. Set `NEXT_PUBLIC_CONVEX_URL` and `CONVEX_DEPLOYMENT` in Project Settings.
