...
1. EXTENSIONS
   ...
   - Only enable an optional extension if it is a must have
...
Migration Overview: We are at step 2 of 4 in our migration process. This second migration (001-structure.sql) focuses ONLY on creating utility functions, reference tables, and core tables. We'll handle relationships, foreign keys, and security in later migrations. Before proceeding, verify that your 000-foundation.sql file includes all necessary extensions, custom schemas, domain types, and enums that will be used by your tables. Do NOT include foreign key constraints, triggers, RLS policies, or indexes in this migration as these will be handled in subsequent migrations.
...
3. CORE TABLES
   ...
   - Do NOT define foreign key constraints (save these for 002-relationships.sql)
   - Do NOT create triggers (save these for 003-security.sql)
   - Do NOT create indexes (save these for 002-relationships.sql)
